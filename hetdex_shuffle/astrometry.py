...
class TangentPlane:
    """Minimal replacement for pyhetdex.coordinates.tangent_projection.TangentPlane

    Methods:
    - raDec2xy(ra, dec) -> returns (x_arcsec, y_arcsec) offsets in arcsec.
    - xy2raDec(x_arcsec, y_arcsec) -> returns (ra_deg, dec_deg).
    """

    def __init__(self, ra0, dec0, pa, x_scale, y_scale, x0=0.0, y0=0.0):
        self.ra0 = float(ra0)
        ...
        self.x0 = float(x0)
        self.y0 = float(y0)
        self._w = wcs.WCS(naxis=2)
        self._w.wcs.crpix = [self.x0, self.y0]
        self._w.wcs.crval = [self.ra0, self.dec0]
        self._w.wcs.ctype = ["RA---TAN", "DEC--TAN"]
        # scale in deg/pix corresponding to arcsec units per pixel
        self._w.wcs.cdelt = [self.x_scale / 3600.0, self.y_scale / 3600.0]
        r = deg2rad(self.pa)
        self._w.wcs.pc = [[cos(r), sin(r)], [-sin(r), cos(r)]]

    def raDec2xy(self, ra, dec):
        xpix, ypix = self._w.wcs_world2pix(ra, dec, 1)
        return xpix * self.x_scale, ypix * self.y_scale

    def xy2raDec(self, x_arcsec, y_arcsec):
        xpix = x_arcsec / self.x_scale
        ypix = y_arcsec / self.y_scale
        return self._w.wcs_pix2world(xpix, ypix, 1)
...
class FPlane(object):
    ...
    def __init__(
        ...
        self._IFU = ifu_class
        self._ifus_by_slot = {}
        # Public iterable of IFUs to mirror pyhetdex API expected by callers
        self.ifus = []
        self._load_fplane(
